...
This document derives and implements the Weibull log-likelihood with optional right censoring. We walk through the probability density function (PDF), survival function S(t), cumulative hazard H(t), and the full log-likelihood step-by-step, with algebraic derivations and interpretation. We also demonstrate this with simulated data. Much of this document comes from the Weibull probability density function (see <https://en.wikipedia.org/wiki/Weibull_distribution>) and the first 100 pages of Paul Allison's Survival Analysis using SAS book.
...
We simulate a bunch of distributions below to visualize this, but first we'll walk through some important math involving the Weibull distribution and derive a log-likelihood for censored data.
...
We need the cumulative hazard function H(t) because it captures the total risk of failure accumulated up to time t. This is important even for censored observations, since knowing that a critter survived up to a certain point still provides useful information about the underlying processes that we are interested in.
...
-   $\boldsymbol{\log h(t_i)}$: Only contributes when the event is observed $\boldsymbol{d_i = 1}$. It reflects the **instantaneous risk**.
...
This then represents a numerical computing problem and parameters can be estimated using maximum likelihood or Bayesian methods (e.g., see Pawitawn 2013; Gelman et al. 2025). You can see how to call this model in the `compare.R` script (note that the relevant function is called `nll_loghaz_shape_scale`).

## References

Paul Allison. Survival Analysis Using SAS: A Practical Guide. 2nd Edition.
